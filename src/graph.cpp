...
static void single_source_dijkstra(
...
Node &DiGraph::add_node(const std::string &id, double length) {
    ...
    Bindings DiGraph::encode_bindings(
    ...
    Sequences
    DiGraph::encode_sequences(const std::vector<std::vector<std::string>> &sequences)
    {
        Sequences ret;
        ret.graph = this;
    ...
    Endpoints DiGraph::encode_endpoints(
        const std::unordered_map<std::string, std::tuple<Eigen::Vector2d, Eigen::Vector2d>>
            &endpoints,
        bool is_wgs84)
    {
        Endpoints ret;
    ...
    std::optional<UbodtRecord> DiGraph::encode_ubodt(const std::string &source_road,
    ...
    std::optional<int64_t> DiGraph::__node_id(const std::string &node) const {
        return indexer_.get_id(node);
    }

    std::optional<std::tuple<int64_t, double>>
    DiGraph::__node_length(const std::string &node) const {
    ...
    std::optional<Path>
    DiGraph::shortest_path(const std::string &source,           //
    ...
    DiGraph::shortest_zigzag_path(const std::string &source,                //
    ...
    DiGraph::shortest_path_to_bindings(
