  ...
  public:
    ...
    void consolidate() {
        ...
        int max_degree = static_cast<int>(
            floor(log(static_cast<double>(n)) / log(static_cast<double>(1 + sqrt(static_cast<double>(5))) / 2)));

        // 1
        A = new FibNode *[max_degree + 2];  // plus two both for indexing to max degree and
...
struct Heap {
    ...
    inline bool contain_node(T index) const { return handle_data.find(index) != handle_data.end(); }
    inline void decrease_key(T index, double value) { heap.decrease_key(handle_data[index], {index, value}); }
