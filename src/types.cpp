...
inline Eigen::VectorXd __offsets(const RowVectors &polyline, double eps = std::numeric_limits<double>::min()) {
    ...
    Eigen::VectorXd norms = (polyline.bottomRows(N - 1) - polyline.topRows(N - 1)).rowwise().norm();
...
inline RowVectors __dirs(const RowVectors &polyline) {
    ...
    Eigen::VectorXd norms2 = (polyline.bottomRows(N - 1) - polyline.topRows(N - 1)).rowwise().norm();
    for (int i = 0; i < N - 1; ++i) {
        if (norms2[i]) {
        ...
        Eigen::Vector3d delta = polyline.row(std::min(r + 1, N - 1)) - polyline.row(l);
